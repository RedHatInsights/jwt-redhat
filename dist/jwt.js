...
var JWT_REDHAT_IDENTIFIER = 'jwt_redhat';
var TOKEN_SURFIX = "_" + JWT_REDHAT_IDENTIFIER + "_token";
var REFRESH_TOKEN_NAME_SURFIX = "_" + JWT_REDHAT_IDENTIFIER + "_refresh_token";
var FAIL_COUNT_NAME_SURFIX = "_" + JWT_REDHAT_IDENTIFIER + "_refresh_fail_count";
var INTERNAL_ROLE = 'redhat:employees';
var TOKEN_NAME = "" + DEFAULT_KEYCLOAK_OPTIONS.clientId + TOKEN_SURFIX;
var INITIAL_JWT_OPTIONS = undefined;
var COOKIE_TOKEN_NAME = TOKEN_NAME;
var REFRESH_TOKEN_NAME = "" + DEFAULT_KEYCLOAK_OPTIONS.clientId + REFRESH_TOKEN_NAME_SURFIX;
var FAIL_COUNT_NAME = "" + DEFAULT_KEYCLOAK_OPTIONS.clientId + FAIL_COUNT_NAME_SURFIX;
...
function reinit() {
    ...
    if (state.keycloak)
        state.keycloak.removeIframeFromDom();
            ...
            if (e && e.data && e.data.type === 'Initialized' && !state.keycloak.authenticated && e.data.authenticated) {
                if (options.clientId === e.data.clientId) {
                    ...
                    if (jwtOptions.reLoginIframeEnabled && jwtOptions.reLoginIframe) {
                        var iframeMessage = { value: null, message: 'reinit' };
...
function keycloakInitSuccess(authenticated) {
    ...
    if (authenticated) {
        ...
        resetKeyCount(FAIL_COUNT_NAME).then(function () {
            startRefreshLoop();
        }).catch(function () {
        ...
        if (INITIAL_JWT_OPTIONS.reLoginIframeEnabled && INITIAL_JWT_OPTIONS.reLoginIframe) {
            var iframeJwtOptions = Object.assign({}, INITIAL_JWT_OPTIONS);
            ...
            var iframeMessage = { value: iframeJwtOptions, message: 'init' };
...
function handleInitEvents() {
    if (events.init.length > 0) {
        events.init.forEach(function (event) {
...
function handleInitErrorEvents() {
    if (events.initError.length > 0) {
        events.initError.forEach(function (event) {
            if (typeof event === 'function') {
                event(Jwt);
            }
        });
...
function handleTokenEvents() {
    while (events.token.length) {
        var event_1 = events.token.shift();
...
function handleTokenMismatchEvents() {
    while (events.tokenMismatch.length) {
        var event_2 = events.tokenMismatch.shift();
...
function handleJwtTokenUpdateFailedEvents() {
    while (events.jwtTokenUpdateFailed.length) {
        var event_3 = events.jwtTokenUpdateFailed.shift();
...
function ssoUrl(isInternal) {
    ...
    switch (location.hostname) {
        ...
        default:
            log('[jwt.js] ENV: ci');
            var subSubDomain = isInternal === true ? 'dev' : 'dev2';
...
function updateToken(force) {
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, failCountPassed(FAIL_COUNT_NAME, FAIL_COUNT_THRESHOLD)];
                case 1:
...
function updateTokenFailure(e) {
    ...
    var userLoginTime = undefined;
    if (initialUserToken) {
        userLoginTime = (+new Date() - initialUserToken.auth_time * 1000) / 1000 / 60 / 60;
    }
    failCountEqualsThreshold(FAIL_COUNT_NAME, FAIL_COUNT_THRESHOLD).then(function (isfailCountEqualsThreshold) {
...
function getStoredTokenValue() {
    var token = lib.store.local.get(TOKEN_NAME);
...
function getCountForKey(key) {
    try {
        return cacheUtils_1.CacheUtils.get(key).then(function (countCache) {
...
function failCountPassed(key, threshold) {
    return getCountForKey(key).then(function (count) {
...
function incKeyCount(key) {
    return getCountForKey(key).then(function (keyCount) {
        var newKeyCount = keyCount + 1;
        var newFailCountCache = {
            value: newKeyCount
        };
        cacheUtils_1.CacheUtils.set(key, newFailCountCache);
        return newKeyCount;
    });
}
/**
...
function resetKeyCount(key) {
    var newSentryLogCountCache = {
        value: 0
    };
    return cacheUtils_1.CacheUtils.set(key, newSentryLogCountCache);
}
var Jwt = {
